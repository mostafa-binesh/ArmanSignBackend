...
class ReportAdmin(admin.ModelAdmin):
    ...
    def move_selected_up(self, request, queryset):
        ...
        for report in queryset:
            ...
            if report.sort_order > 1:
                report.sort_order = F('sort_order') - 1
                report.save()
    ...
    def move_selected_down(self, request, queryset):
        ...
        max_sort_order_item = Report.objects.all().order_by('-sort_order').first()
        max_sort_order = Report.objects.all().order_by('-sort_order').first().sort_order
        for report in queryset:
            # Prevent going beyond the maximum sort order
            if report.sort_order <= max_sort_order and report.id != max_sort_order_item.id:
                report.sort_order = F('sort_order') + 1
                report.save()
